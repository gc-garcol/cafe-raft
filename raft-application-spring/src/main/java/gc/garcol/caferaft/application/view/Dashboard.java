...
public class Dashboard {
    ...
    public String cluster(Model model) {
        var nodes = java.util.stream.IntStream.range(0, clusterProperty.getNodes().size())
                .mapToObj(i -> new NodeInfo(i, clusterProperty.getNodes().get(i))).map(Object::toString)
        ...
        model.addAttribute("totalLogs",
                logManager.segments.stream().map(Segment::getSize).reduce(Long::sum).orElse(0L));
