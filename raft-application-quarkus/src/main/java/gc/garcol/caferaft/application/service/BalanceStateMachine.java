        ...
        var results = batchBalanceCommand.getCommands().stream().map(command -> {
            try {
                if (command.getCreateBalanceCommand() != null) {
                    ...
                    return new ModifyBalanceResponse(command.getCorrelationId(), 200, "OK");
                }
                ...
                return new ModifyBalanceResponse(command.getCorrelationId(), 400, "Command not found");
            } catch (Exception e) {
                return new ModifyBalanceResponse(command.getCorrelationId(), 400, e.getMessage());
            }
        }).toList();
