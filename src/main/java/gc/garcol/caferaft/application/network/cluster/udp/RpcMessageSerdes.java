...
public class RpcMessageSerdes {
    ...
    private static final Map<Integer, Class<? extends ClusterRpc>> RPC_CLUSTER_TYPES = Map.of(
        ...
        UdpChannelConstant.RPC_TYPE_VOTE_REQUEST, VoteRequest.class,
        UdpChannelConstant.RPC_TYPE_VOTE_RESPONSE, VoteResponse.class
    );
    ...
    public byte[] toBytes(ClusterRpc clusterRpc) {
        ...
        return ByteBuffer.allocate(header.length + payload.length)
            .put(header)
            .put(payload)
            .array();
    }
