...
public class RaftLogicHandlerImpl implements RaftLogicHandler {
    ...
    private void candidateLogic() {
        ...
        if (currentTime >= raftState.getElectionTimeout()) {
            ...
            var newElectionTimeout = System.currentTimeMillis() + ThreadLocalRandom.current()
                    .nextInt(clusterProperty.getElectionTimeoutMs()[0], clusterProperty.getElectionTimeoutMs()[1]);
        ...
        if (raftState.getHeartbeatTimeout() < System.currentTimeMillis()) {
            ...
            this.broadcastService.broadcast(nodeId -> {
                ...
                raftState.getLeaderVolatileState().getNextSendLogTime()
                        .put(nodeId, System.currentTimeMillis() + clusterProperty.getNextSendLogTimeoutMs());
                ...
                while (logEntries.size() < batchSize && currentAppendPosition != null && currentAppendPosition.compareTo(
                        lastPosition) <= 0) {
                    if (!INITIAL_POSITION.equals(currentAppendPosition)) {
                        LogEntry logEntry = logManager.getLog(currentAppendPosition.term(),
                                currentAppendPosition.index());
                        if (logEntry == null) {
                            break;
                        }
                        logEntries.add(
                                new LogEntryRequest(logEntry.getPosition(), commandSerdes.type(logEntry.getCommand()),
    ...
    Tuple<Position, CommandResponse> applyStateMachine() {
        ...
        if (raftState.getVolatileState().getLastApplied()
                .compareTo(raftState.getVolatileState().getCommitPosition()) < 0) {
    ...
    private void handleExpiredRepliers(Position currentPosition) {
        while (!repliers.isEmpty() && repliers.getFirst().position().compareTo(currentPosition) < 0) {
            ...
            replyPublisher.publish(() -> replier.replier()
                    .complete(new CommonErrorResponse(408, "Request timeout, old-request is not processed")));
        }
    }
    ...
    record Tuple<T1, T2>(T1 _1, T2 _2) {
    }
}
