...
public class ClusterRpcHandlerImpl implements ClusterRpcHandler {
    ...
    private void handleAppendEntryRequest(AppendEntryRequest request) {
        ...
        if (request.getTerm() < raftState.getPersistentState().getCurrentTerm()) {
            ...
            commonExecutorPool.execute(
                    () -> rpcNetworkOutbound.appendEntryResponse(request.getSender(), appendEntryResponse));
            raftState.setHeartbeatTimeout(System.currentTimeMillis() + clusterProperty.getHeartbeatTimeoutMs());
            return;
        }
        ...
        if (!INITIAL_POSITION.equals(request.getPreviousPosition())) {
            LogEntry previousLog = logManager.getLog(request.getPreviousPosition().term(),
                    request.getPreviousPosition().index());
            if (previousLog == null) {
                appendEntryResponse.setSuccess(false);
                appendEntryResponse.setConflictPosition(logManager.lastPosition());
                commonExecutorPool.execute(
            ...
            if (request.isHeartbeat()) {
                ...
                for (LogEntryRequest logEntry : request.getEntries()) {
                    if (existedPosition == null) {
                        ...
                        logManager.appendLog(logEntry.getPosition().term(),
                                commandSerdes.fromBytes(logEntry.getType(), logEntry.getCommand()));
                        continue;
                    }
        ...
        Position matchedPosition = request.isHeartbeat() ?
                logManager.lastPosition() :
                request.getEntries().getLast().getPosition();
    ...
    private void handleAppendEntryResponse(AppendEntryResponse response) {
        ...
        raftState.getLeaderVolatileState().getNextSendLogTime()
                .put(response.getSender(), System.currentTimeMillis() + clusterProperty.getNextSendLogTimeoutMs());

        // [Docs]: If AppendEntries fails because of log inconsistency:
        // decrement nextIndex and retry (§5.3)
        if (!response.isSuccess()) {
            var nextAppendPosition = raftState.getLeaderVolatileState().getNextAppendPositions()
                    .get(response.getSender());
            ...
            if (raftState.getLeaderVolatileState().getMatchPositions().get(response.getSender())
                    .compareTo(downGradePosition) <= 0) {
                raftState.getLeaderVolatileState().getNextAppendPositions()
                        .put(response.getSender(), downGradePosition);
            }
            return;
        }
        ...
        // follower (§5.3)
        if (raftState.getLeaderVolatileState().getMatchPositions().get(response.getSender())
                .compareTo(response.getMatchedPosition()) < 0) {
            ...
            raftState.getLeaderVolatileState().getMatchPositions()
                    .put(response.getSender(), response.getMatchedPosition());
            raftState.getLeaderVolatileState().getNextAppendPositions()
                    .put(response.getSender(), newNextAppendPosition);
        }

        // [Docs]: • If there exists an N such that N > commitIndex, a majority
        // of matchIndex[i] ≥ N, and log[N].term == currentTerm:
        // set commitIndex = N (§5.3, §5.4).
        List<Position> sortedMatchedPositions = raftState.getLeaderVolatileState().getMatchPositions().values().stream()
                .sorted().toList();
    ...
    private void handleVoteRequest(VoteRequest request) {
        ...
        boolean isValidVoteFor = persistentState.getVotedFor() == null || persistentState.getVotedFor()
                .id() == request.getCandidateId().id();
    ...
    private void handleVoteResponse(VoteResponse response) {
        ...
        if (!RaftRole.LEADER.equals(raftState.getRole()) && totalApproved > raftState.getPersistentState()
                .getTotalNodes() / 2) {
